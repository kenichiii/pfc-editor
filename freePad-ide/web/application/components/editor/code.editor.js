...
$.freePad.editor = {
        ...
        openfile:function(file_path,root,ext,fileid,realpath) {
            
            
            var freePadEditorThat = this;
            
            $.freePad.ui.showWaitingBox();    

         $.post(freePadEditorThat.config.getFileContentsUrl,{path:file_path,root:root},function(code){
                                                                                
           if(code.encoding==='UNKWN')
             {
                  $.freePad.ui.confirm("FILE HAS UNKNOWN ENCODING!<br>Open anyway and risk wrong convert and save?",function(){
                      freePadEditorThat.addFile(code,file_path,root,ext);         
                  });    
             }
           else if(code.saveToOpen=='no')
             {
                  $.freePad.ui.confirm(root+" <b>"+file_path+"</b><br>FILE HAS "+code.encoding+" ENCODING!<br>File dont pass check to be converted to utf-8 and back.<br>Open anyway and most probably get wrong characters encoding convert?",function(){
                      freePadEditorThat.addFile(code,file_path,root,ext);         
                  });    
             }
           ...
           $.freePad.ui.hideWaitingBox();       
      
          },'json').fail(function(){
                    $.freePad.ui.hideWaitingBox();    
                    $.freePad.ui.alert('ajax-error',{type:'err'});
                });  
        },
        ...
        ui: {
                ...
                openDialogsTabsListeners: function(obj,isPage,code,file_path,root,ext) {
                       ...
                       $('.pfc-editor-dialog-tab-close').first().unbind('click').click(function(){
                           ...
                           $.freePad.editor.closeDialog(id.replace('#',''));
                       ...
                       $('.pfc-editor-dialog-tab-title').first().unbind('click').click(function(){
                           ...
                           $.freePad.editor.ui.removeActiveState(); 
                           $.freePad.editor.activateTab(id.replace('#',''));
                         
                           if(!isPage && $(this).parent().hasClass('file')) {
                              $.freePad.editor.run_last_modification_checker(code,file_path,root,ext);                                                                                                           
                           }
            ...
            addPage: function(page) {                                
                ...
                {
                    page.counter++;
                    this.addActiveDialogTab(page.title+' #'+page.counter,page.pageid+'_'+page.counter);
                    $.freePad.ui.showWaitingBox();
                    ...
                    $.get(page.config.url,params,function(html){                        
                        ...
                        $.freePad.editor.addActivePageBody(page.pageid+'_'+page.counter,html);
                        
                        $.freePad.editor.ui.openDialogsTabsListeners(page,true);
                        
                        page.init(page.pageid+'_'+page.counter);
                        $.freePad.editor.activateTab(page.pageid+'_'+page.counter);
                        
                        $.freePad.addToEditorBar({type:'page',bean:page});
                        
                        $.freePad.ui.hideWaitingBox();
                    })
                    .fail(function(){
                        $.freePad.editor.closeDialog(page.pageid+'_'+page.counter);
                        $.freePad.ui.hideWaitingBox();
                        $.freePad.ui.alert('ajax-error',{type:'err'});
                    });
                }
                ...
                {
                       $.freePad.editor.activateTab(page.pageid);
                }
                else {
                  if($("#pfc-editor-dialogs-heads a[href='#"+page.pageid+"']").length===0)
                  {   
                    $.freePad.editor.addActiveDialogTab(page.title,page.pageid,null,ext);
                    $.freePad.ui.showWaitingBox();
                    params = page.config.urlParams?page.config.urlParams:{};
                    $.get(page.config.url,params,function(html){                        
                        ...
                        $.freePad.editor.addActivePageBody(page.pageid,html);
                    ...
                    .fail(function(){
                        $.freePad.editor.closeDialog(page.pageid);
                        $.freePad.ui.hideWaitingBox();
                        $.freePad.ui.alert('ajax-error',{type:'err'});
                    });
                }
            ...
            addFile: function(code,file_path,root,ext) {
                ...
                {                    
                   ...
                   //chrome needs -30px, IE and FF not
                    ...
                    $.freePad.editor.ui.openDialogsTabsListeners({},false,code,file_path,root,ext);
                    
                    $.freePad.editor.activateTab('file_'+code.id);                                         
                    
                    $.freePad.editor.addToEditorBar('file',{id:code.id,type:'file',file_path:file_path,root:root,ext:ext}); 
                 ...
                 if(code.base=='sandbox-src')
                   {
                        ...
                        $t.find('.pfc-editor-file-button-run').click(function(){
                          ...
                          {
                             $.freePad.ui.dialog(that.config.sandboxUrl+code.path,{modal:0,title:code.name});                                                          
                          }
                                           return false;
                                         });
                                    ...
                                    if(json.uptodate==='no' && that.ceditors[code.id].ignore != 'no' && !that.ceditors[code.id].waitingReload && that.ceditors[code.id].lu!=json.actualTime)
                                      {
                                        ...
                                        $.freePad.ui.confirm('Actual file has changed on the background.<br>Reload file to last actual version?',
                                         function(){
                                          that.ceditors[code.id].waitingReload = false;
                                     ...
                                     else if(json.uptodate==='not-exists' && that.ceditors[code.id].ignore != 'not-exists' && !that.ceditors[code.id].waitingClose)
                                       {
                                        ...
                                        $.freePad.ui.confirm('Actual file has been deleted on the background.<br>Close file?',
                                         function(){
            ...
            closeDialog: function(id) {
              var that = this;
                        ...
                        var index = $("#pfc-editor-dialogs-heads a[href='#"+id+"']")
                                .parent().index();
                        
                        $("#pfc-editor-dialogs-heads a[href='#"+id+"']")
                                .parent().remove();
                
                $.freePad.removeFromEditorBar(index);
                ...
                $.post(that.config.appSaveFileUrl,{
                    ...
                    confirmOverwrite: confirmOverwrite ? 'yes' : 'no'
                     ...
                     if(res.succ=='yes')
                     {
                        ...
                        $.freePad.ui.alert('File successfully saved',{autohide:true,delayAutoHide:1,modal:0});
                     ...
                     else if(res.succ=='waiting')
                     {
                       if(res.type === 'cant-succ-convert')
                         {
                           $.freePad.ui.confirm(that.ceditors[test].root+" <b>"+that.ceditors[test].path+"</b><br>FILE HAS "+that.ceditors[test].encoding+" ENCODING.<br>File contents dont pass check to be converted to this and back to UTF-8.<br>Save anyway with most probably wrong encoded characters?",
                                         function(){
                       ...
                       $.freePad.ui.alert(res.msg,{type:'err'});
                     }
                    ...
                    $.freePad.ui.alert('action-error',{type:'err'});
                });
                
        }
        else $.freePad.ui.alert('No file is active to save',{type:'err'});
