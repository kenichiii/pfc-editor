...
$.idePad.editor = {
        ...
        openfile:function(file_path,root,ext,fileid,realpath) {
            ...
            var idePadEditorThat = this;
            
                $.idePad.ui.showWaitingBox();    

                $.post(freePadEditorThat.config.getFileContentsUrl,{path:file_path,root:root},function(code){
                                                                                
                    if(code.encoding==='UNKWN')
                    {
                        $.idePad.ui.confirm("FILE HAS UNKNOWN ENCODING!<br>Open anyway and risk wrong convert and save?",function(){
                            idePadEditorThat.addFile(code,file_path,root,ext);         
                        });    
                    }
                    else if(code.saveToOpen=='no')
                    {
                        $.idePad.ui.confirm(root+" <b>"+file_path+"</b><br>FILE HAS "+code.encoding+" ENCODING!<br>File dont pass check to be converted to utf-8 and back.<br>Open anyway and most probably get wrong characters encoding convert?",function(){
                        idePadEditorThat.addFile(code,file_path,root,ext);         
                        });    
                    }
                    ...
                    $.idePad.ui.hideWaitingBox();       
      
                },'json').fail(function(){
                    $.idePad.ui.hideWaitingBox();    
                    $.idePad.ui.alert('ajax-error',{type:'err'});
                });  
        },
        ...
        ui: {
                ...
                openDialogsTabsListeners: function(obj,isPage,code,file_path,root,ext) {
                       ...
                       $('.pfc-editor-dialog-tab-close').first().unbind('click').click(function(){
                           ...
                           $.idePad.editor.closeDialog(id.replace('#',''));
                       ...
                       $('.pfc-editor-dialog-tab-title').first().unbind('click').click(function(){
                           ...
                           $.idePad.editor.activateTab(id.replace('#',''));
                         
                           if(!isPage && $(this).parent().hasClass('file')) {
                              $.idePad.editor.run_last_modification_checker(code,file_path,root,ext);                                                                                                           
                           }
            ...
            addPage: function(page) {                                
                ...
                if(page.multi)
                {
                    ...
                    $.get(page.config.url,params,function(html){                        
                        ...
                        $.idePad.editor.addActivePageBody(page.pageid+'_'+page.counter,html);
                        
                        $.idePad.editor.ui.openDialogsTabsListeners(page,true);
                        
                        page.init(page.pageid+'_'+page.counter);
                        $.idePad.editor.activateTab(page.pageid+'_'+page.counter);
                        
                        $.idePad.addToEditorBar({type:'page',bean:page});
                        
                        $.idePad.ui.hideWaitingBox();
                    })
                    .fail(function(){
                        $.idePad.editor.closeDialog(page.pageid+'_'+page.counter);
                        $.idePad.ui.hideWaitingBox();
                        $.idePad.ui.alert('ajax-error',{type:'err'});
                    });
                }
                ...
                {
                       $.idePad.editor.activateTab(page.pageid);
                }
                else {
                  if($("#pfc-editor-dialogs-heads a[href='#"+page.pageid+"']").length===0)
                  {   
                    $.idePad.editor.addActiveDialogTab(page.title,page.pageid,null,ext);
                    $.idePad.ui.showWaitingBox();
                    params = page.config.urlParams?page.config.urlParams:{};
                    $.get(page.config.url,params,function(html){                        
                        ...
                        $.idePad.editor.addActivePageBody(page.pageid,html);
                    ...
                    .fail(function(){
                        $.idePad.editor.closeDialog(page.pageid);
                        $.idePad.ui.hideWaitingBox();
                        $.idePad.ui.alert('ajax-error',{type:'err'});
                    });
                }
            ...
            addFile: function(code,file_path,root,ext) {
                ...
                {                    
                   ...
                   //chrome needs -30px, IE and FF not
                    ...
                    $.idePad.editor.ui.openDialogsTabsListeners({},false,code,file_path,root,ext);
                    
                    $.idePad.editor.activateTab('file_'+code.id);                                         
                    
                    $.idePad.editor.addToEditorBar('file',{id:code.id,type:'file',file_path:file_path,root:root,ext:ext}); 
                 ...
                 if(code.base=='sandbox-src')
                   {
                        ...
                        $t.find('.pfc-editor-file-button-run').click(function(){
                          ...
                          {
                             $.idePad.ui.dialog(that.config.sandboxUrl+code.path,{modal:0,title:code.name});                                                          
                          }
                                           return false;
                                         });
                                    ...
                                    if(json.uptodate==='no' && that.ceditors[code.id].ignore != 'no' && !that.ceditors[code.id].waitingReload && that.ceditors[code.id].lu!=json.actualTime)
                                      {
                                        ...
                                        $.idePad.ui.confirm('Actual file has changed on the background.<br>Reload file to last actual version?',
                                         function(){
                                          that.ceditors[code.id].waitingReload = false;
                                     ...
                                     else if(json.uptodate==='not-exists' && that.ceditors[code.id].ignore != 'not-exists' && !that.ceditors[code.id].waitingClose)
                                       {
                                        ...
                                        $.idePad.ui.confirm('Actual file has been deleted on the background.<br>Close file?',
                                         function(){
            ...
            closeDialog: function(id) {
              var that = this;
                ...
                $.idePad.removeFromEditorBar(index);
                ...
                $.post(that.config.appSaveFileUrl,{
                    ...
                    confirmOverwrite: confirmOverwrite ? 'yes' : 'no'
                     ...
                     if(res.succ=='yes')
                     {
                        ...
                        $.idePad.ui.alert('File successfully saved',{autohide:true,delayAutoHide:1,modal:0});
                     ...
                     else if(res.succ=='waiting')
                     {
                       if(res.type === 'cant-succ-convert')
                         {
                           $.idePad.ui.confirm(that.ceditors[test].root+" <b>"+that.ceditors[test].path+"</b><br>FILE HAS "+that.ceditors[test].encoding+" ENCODING.<br>File contents dont pass check to be converted to this and back to UTF-8.<br>Save anyway with most probably wrong encoded characters?",
                                         function(){
                       ...
                       $.idePad.ui.alert(res.msg,{type:'err'});
                     }
                    ...
                    $.idePad.ui.alert('action-error',{type:'err'});
                });
                
        }
        else $.idePad.ui.alert('No file is active to save',{type:'err'});
